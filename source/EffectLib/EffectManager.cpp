...
BOOL CEffectManager::RegisterEffect(const char * c_szFileName,bool isExistDelete,bool isNeedCache)
{
...
#ifdef ENABLE_REGISTER_EFFECT_LOG
	Tracenf("CEffectManager::RegisterEffect - Debug LoadScript(%s)", c_szFileName);
#endif
	...
	if (m_kEftDataMap.end() != itor)
	{
		...
		{
#ifdef ENABLE_REGISTER_EFFECT_LOG
			Tracenf("CEffectManager::RegisterEffect - m_kEftDataMap.find [%s] Already Exist", c_szFileName);
#endif
			return TRUE;
		}
	}
