...
// NEW_INTERFACE
PyObject * wndMgrHideCoverButton(PyObject * poSelf, PyObject * poArgs)
{
	UI::CWindow * pWindow;
	if (!PyTuple_GetWindow(poArgs, 0, &pWindow))
		return Py_BuildException();
	...
	UI::CSlotWindow * pSlotWin = (UI::CSlotWindow *)pWindow;
	pSlotWin->HideCoverButton(iSlotIndex);

	return Py_BuildNone();
}
// END_OF_NEW_INTERFACE
...
PyObject* wndTextDisableDynamicFont(PyObject* poSelf, PyObject* poArgs)
{
	UI::CWindow* pWindow;
	...
	if (!PyTuple_GetBoolean(poArgs, 1, &byFlag))
		return Py_BuildException();

	dynamic_cast<UI::CTextLine*>(pWindow)->DisableDynamicFont(byFlag);
	return Py_BuildNone();
}
#endif
...
#if defined(ENABLE_HIGH_LIGHT_IMAGE) || defined(ENABLE_DAILY_GIFT_SYSTEM)
...
PyObject * wndMgrSetSlotImage(PyObject * poSelf, PyObject * poArgs) // item +9
{
	UI::CWindow * pWin;
	if (!PyTuple_GetWindow(poArgs, 0, &pWin))
	  return Py_BuildException();
...
void initwndMgr()
{
	static PyMethodDef s_methods[] =
	{
		...
		// NEW_INTERFACE
		{ "HideCoverButton",			wndMgrHideCoverButton,				METH_VARARGS },
		// END_OF_NEW_INTERFACE
...
#ifdef ENABLE_DYNAMIC_FONT
		{ "DisableDynamicFont", wndTextDisableDynamicFont, METH_VARARGS },
#endif
		// NumberLine
...
// "aslan hunt"
		...
		// NEW_INTERFACE
		{ "Over",						wndButtonOver,						METH_VARARGS }, // Owaran 2017.10.07
		// END_OF_NEW_INTERFACE
