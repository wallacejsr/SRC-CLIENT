...
void CPythonMiniMap::Update(float fCenterX, float fCenterY)
{
	...
	for(i = rkChrMgr.CharacterInstanceBegin(); i!=rkChrMgr.CharacterInstanceEnd(); ++i)
	{
		...
		if (pkInstEach->IsPC() && !pkInstEach->IsInvisibility())
		{
			...
			if (pkInstEach->IsBuffNPC(pkInstEach->GetVirtualID())) {
		...
		{
#ifdef ENABLE_DUNGEON_BOSS_ICON_IN_MAP
			if (GetDungeonNPC(pkInstEach->GetRace()))
			{
...
bool CPythonMiniMap::Create()
{
	...
	const std::string strPlayerMark = strImageRoot + "minimap/whitemark.tga";
	const std::string strWhiteMark = strImageRoot + "minimap/whitemark.tga";
#ifdef ENABLE_DUNGEON_BOSS_ICON_IN_MAP
	...
	pSubImage = (CGraphicSubImage*)CResourceManager::Instance().GetResourcePointer(strDungeonMark.c_str());
	m_DungeonMark.SetImagePointer(pSubImage);
#endif
...
void CPythonMiniMap::ClearAtlasMarkInfo()
{
	...
	m_AtlasSafeZoneInfoVector.clear();
#endif
#ifdef ENABLE_DUNGEON_BOSS_ICON_IN_MAP
...
)
{
	...
	if (GetDungeonNPC(mobVnum))
	{
		aAtlasMarkInfo.time = time;
...
#ifdef ENABLE_BATTLE_ROYALE
void CPythonMiniMap::ClearBattleRoyaleZone()
{
	m_AtlasSafeZoneInfoVector.clear();
}

void CPythonMiniMap::RegisterBattleRoyaleZone(long lx, long ly, long lEndX, long lEndY, BYTE bType)
{
	if (bType == 1)
		ClearBattleRoyaleZone();

	//TraceError("RegisterBattleRoyaleZone %d %d | %d %d", lx, ly, lEndX, lEndY);

	int iWidth = (lEndX - lx) / 100;
	int iHeight = (lEndY - ly) / 100;

	int iCenterX = (lx/100) + (iWidth / 2);
	int iCenterY = (ly/100) + (iHeight / 2);

	//minimal size
	int iMinimumWidth = 40;
	int iMinimumHeight = 40;

	if (iWidth < iMinimumWidth)
	{
		lx		= (iCenterX - (iMinimumWidth / 2)) * 100;
		lEndX	= (iCenterX + (iMinimumWidth / 2)) * 100;
	}

	if (iHeight < iMinimumHeight)
	{
		ly		= (iCenterY - (iMinimumHeight / 2)) * 100;
		lEndY	= (iCenterY + (iMinimumHeight / 2)) * 100;
	}
	//minimal size

	TAtlasMarkInfo aAtlasMarkInfo;

	aAtlasMarkInfo.m_dwID = bType;

	aAtlasMarkInfo.m_fX = float(lx);
	aAtlasMarkInfo.m_fY = float(ly);
	float fEndX = float(lEndX);
	float fEndY = float(lEndY);

	aAtlasMarkInfo.m_strText = "safe zone";

	aAtlasMarkInfo.m_fScreenX = aAtlasMarkInfo.m_fX / m_fAtlasMaxX * m_fAtlasImageSizeX - (float)m_WhiteMark.GetWidth() / 2.0f;
	aAtlasMarkInfo.m_fScreenY = aAtlasMarkInfo.m_fY / m_fAtlasMaxY * m_fAtlasImageSizeY - (float)m_WhiteMark.GetHeight() / 2.0f;

	aAtlasMarkInfo.m_fMiniMapX = fEndX / m_fAtlasMaxX * m_fAtlasImageSizeX - (float)m_WhiteMark.GetWidth() / 2.0f;
	aAtlasMarkInfo.m_fMiniMapY = fEndY / m_fAtlasMaxY * m_fAtlasImageSizeY - (float)m_WhiteMark.GetHeight() / 2.0f;

	aAtlasMarkInfo.m_byType = TYPE_NPC;

	m_AtlasSafeZoneInfoVector.push_back(aAtlasMarkInfo);
}

bool CPythonMiniMap::GetAtlasXY(float fScreenX, float fScreenY, float * pReturnPosX, float * pReturnPosY)
{
...
#ifdef ENABLE_BATTLE_ROYALE
	STATEMANAGER.SetRenderState(D3DRS_TEXTUREFACTOR, CInstanceBase::GetIndexedNameColor(CInstanceBase::NAMECOLOR_NPC));
	m_AtlasMarkInfoVectorIterator = m_AtlasSafeZoneInfoVector.begin();
	while (m_AtlasMarkInfoVectorIterator != m_AtlasSafeZoneInfoVector.end())
	{
		TAtlasMarkInfo & rAtlasMarkInfo = *m_AtlasMarkInfoVectorIterator;

		if (rAtlasMarkInfo.m_dwID == 1) //current zone
		{
			CPythonGraphic::Instance().SetDiffuseColor(0.5411f, 0.7254f, 0.5568f, 1.0f);
			CPythonGraphic::Instance().RenderBox2d(rAtlasMarkInfo.m_fScreenX,
