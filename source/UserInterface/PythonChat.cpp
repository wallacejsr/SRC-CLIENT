...
void CPythonChat::AppendChat(int iType, const char * c_szChat)
{
	...
	if (iType == CHAT_TYPE_SHOUT)
	{
		...
		int position = strContent.find(isEmpireFlag);
		...
		if (position != std::string::npos)
		{
			...
			if (strlen(c_szEmpire.c_str()) > 0)
			{
				char buf[128];
				sprintf(buf, "d:/ymir work/ui/empire/empire_%s_256.tga", c_szEmpire.c_str());
