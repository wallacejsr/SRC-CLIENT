...
void CInstanceBase::OnSelected()
{
	...
	if (IsPC() || IsNPC())
	{
		...
		else if (m_dwEmpireID == 3)//Jinno
		{
			__AttachSelectEffectJinno();
		}
	}
#else
...
bool CInstanceBase::Create(const SCreateData& c_rkCreateData)
{
...
#ifdef ENABLE_IMMUNE_BOSS
	SetInstanceImmuneBoss(CPythonNonPlayer::Instance().IsImmuneBossByVnum(c_rkCreateData.m_dwRace));
#endif
	...
	if (c_rkCreateData.m_dwRace == 10)
	{
		SetInstanceType(CActorInstance::TYPE_PC);
		SetRace(7);
		SetHair(c_rkCreateData.m_dwHair);
		SetWeapon(c_rkCreateData.m_dwWeapon);
		m_GraphicThingInstance.SetBuff(10);
	}
	if (c_rkCreateData.m_dwRace == 11)
	{
		SetInstanceType(CActorInstance::TYPE_PC);
		SetRace(3);
		SetHair(c_rkCreateData.m_dwHair);
		SetWeapon(c_rkCreateData.m_dwWeapon);
		m_GraphicThingInstance.SetBuff(11);
	}
#endif
...
int CInstanceBase::GetHorseVnum() //_marty.5.7
{
	return m_kHorse.GetActorPtr() ? m_kHorse.GetActorPtr()->GetRace() : 0;
}
...
#ifdef ENABLE_IMMUNE_BOSS
void CInstanceBase::SetInstanceImmuneBoss(bool bIsImmuneBoss)
{
	m_GraphicThingInstance.SetActorImmuneBossState(bIsImmuneBoss);
}
#endif
...
bool CInstanceBase::IsAttackableInstance(CInstanceBase& rkInstVictim)
{
	...
	else if (IsPC())
	{
		...
		{
#ifdef ENABLE_ASLAN_BUFF_NPC_SYSTEM
			if (rkInstVictim.IsBuffNPC(rkInstVictim.GetVirtualID()))
				return false;
#endif
			if (GetDuelMode())
			{
...
#ifdef ENABLE_ASLAN_BUFF_NPC_SYSTEM
BOOL CInstanceBase::IsBuffNPC(DWORD iVirtualID)
{
	...
	if (pMobTable->dwVnum == 10 || pMobTable->dwVnum == 11)
		return true;
	else
		return false;
}
...
void CInstanceBase::__ClearWeaponRefineEffect()
{
	...
	if (m_swordRefineEffectChristmasRight)
	{
		__DetachEffect(m_swordRefineEffectChristmasRight);
		m_swordRefineEffectChristmasRight = 0;
	}
	if (m_swordRefineEffectChristmasLeft)
	{
		__DetachEffect(m_swordRefineEffectChristmasLeft);
		m_swordRefineEffectChristmasLeft = 0;
	}
#endif
}
...
UINT CInstanceBase::__GetRefinedEffect(CItemData* pItem)
{
		...
		if (pItem->GetSubType() == CItemData::ARMOR_BODY)
		{
			...
			)
			{
				...
				__AttachEffect(EFFECT_REFINED+EFFECT_BODYARMOR_SPECIAL2);
					...
					(20100 <= vnum && vnum <= 20109) // Warrior
					|| (20350 <= vnum && vnum <= 20359) // Ninja
					|| (20600 <= vnum && vnum <= 20609) // Sura
					|| (20850 <= vnum && vnum <= 20859) // Shaman
			)
			{
				__AttachEffect(EFFECT_REFINED+EFFECT_BODYARMOR_PURPLE);
			}
			else if (
					(20110 <= vnum && vnum <= 20119) // Warrior
					|| (20360 <= vnum && vnum <= 20369) // Ninja
					|| (20610 <= vnum && vnum <= 20619) // Sura
					|| (20860 <= vnum && vnum <= 20869) // Shaman
			)
			{
				__AttachEffect(EFFECT_REFINED+EFFECT_BODYARMOR_ORANGE);
			}

			else if (
					(19290 <= vnum && vnum <= 19899) // Todas Ol?picas
			)
			{
				__AttachEffect(EFFECT_REFINED+EFFECT_BODYARMOR_BLACK);
			}
			else if (
					(20120 <= vnum && vnum <= 20129) // Warrior
					|| (20370 <= vnum && vnum <= 20379) // Ninja
					|| (20620 <= vnum && vnum <= 20629) // Sura
					|| (20870 <= vnum && vnum <= 20879) // Shaman
			)
			{
				__AttachEffect(EFFECT_REFINED+EFFECT_BODYARMOR_RED);
			}
#endif
		}
	...
	case CItemData::ITEM_TYPE_COSTUME:
		...
		case CItemData::WEAPON_SWORD:
			if (vnum == 40500 || vnum == 40501) {
				m_swordRefineEffectChristmasLeft = EFFECT_REFINED + EFFECT_SWORD_CHRISTMAS;
			}
			else {
				m_swordRefineEffectChristmasLeft = EFFECT_REFINED + EFFECT_REFINED_NONE;
			}
			break;
		case CItemData::WEAPON_DAGGER:
			if (vnum == 40502){
				m_swordRefineEffectChristmasRight = EFFECT_REFINED + EFFECT_DAGGER_CHRISTMAS;
				m_swordRefineEffectChristmasLeft = EFFECT_REFINED + EFFECT_DAGGER_CHRISTMAS_LEFT;
			}
			else{
				m_swordRefineEffectChristmasRight = EFFECT_REFINED + EFFECT_REFINED_NONE;
				m_swordRefineEffectChristmasLeft = EFFECT_REFINED + EFFECT_REFINED_NONE;
			}
			break;
		case CItemData::WEAPON_BOW:
			if (vnum == 40503){
				m_swordRefineEffectChristmasRight = EFFECT_REFINED + EFFECT_BOW_CHRISTMAS;
			}
			else{
				m_swordRefineEffectChristmasRight = EFFECT_REFINED + EFFECT_REFINED_NONE;
			}
		...
		case CItemData::WEAPON_TWO_HANDED:
			if (vnum == 40504) {
				m_swordRefineEffectChristmasRight = EFFECT_REFINED + EFFECT_TWOHAND_CHRISTMAS;
			}
			else {
				m_swordRefineEffectChristmasRight = EFFECT_REFINED + EFFECT_REFINED_NONE;
			}
		...
		case CItemData::WEAPON_BELL:
			if (vnum == 40505){
				m_swordRefineEffectChristmasRight = EFFECT_REFINED + EFFECT_BELL_CHRISTMAS;
			}
			else{
				m_swordRefineEffectChristmasRight = EFFECT_REFINED + EFFECT_REFINED_NONE;
			}
		...
		case CItemData::WEAPON_FAN:
			if (vnum == 40506){
				m_swordRefineEffectChristmasRight = EFFECT_REFINED + EFFECT_FAN_CHRISTMAS;
				if (m_kHorse.IsMounting()){
					m_swordRefineEffectChristmasLeft = EFFECT_REFINED + EFFECT_FAN_CHRISTMAS_LEFT;
				}
			}
			else{
				m_swordRefineEffectChristmasRight = EFFECT_REFINED + EFFECT_REFINED_NONE;
		...
		if (m_swordRefineEffectChristmasRight)
			m_swordRefineEffectChristmasRight = __AttachEffect(m_swordRefineEffectChristmasRight);
		if (m_swordRefineEffectChristmasLeft)
			m_swordRefineEffectChristmasLeft = __AttachEffect(m_swordRefineEffectChristmasLeft);
#endif
		...
		if (pItem->GetSubType() == CItemData::COSTUME_BODY)
		{
			...
			if (
					(41027 <= vnum && vnum <= 41028) // Traje Imperial
			)
			{
				__AttachEffect(EFFECT_REFINED+EFFECT_BODYARMOR_COSTUME_IMPERIAL);
			}
#endif

#ifdef BODY_COSTUME_WITH_EFFECT_GVG
			else if (
					(vnum == 11919) // Traje GvG
			)
			{
				__AttachEffect(EFFECT_REFINED+EFFECT_BODYARMOR_BLACK);
			}
			...
			m_armorRefineEffect = EFFECT_REFINED+EFFECT_BODYARMOR_COSTUME;
			__AttachEffect(m_armorRefineEffect);
			break;
		}
