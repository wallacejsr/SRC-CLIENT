...
void CPythonNetworkStream::ServerCommand(char * c_szCommand)
{
	...
	{
		CPythonPlayer::Instance().AutoHuntSettings(c_szCommand);
	}
#endif
	// CUBE
	...
	else if (!strcmpi(szCmd, "BINARY_SetRankingListPersonalStats"))
	{
		BYTE byRankingType = atoi(TokenVector[1].c_str());
		const std::string& szName = TokenVector[2].c_str();
		DWORD dwEmpire = atoi(TokenVector[3].c_str());
		long long iCount = atoi(TokenVector[4].c_str());
		int iPosition = atoi(TokenVector[5].c_str());
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_SetRankingListPersonalStats", Py_BuildValue("(isiLi)", byRankingType, szName.c_str(), dwEmpire, iCount, iPosition));
	}
#endif
	else
	{
