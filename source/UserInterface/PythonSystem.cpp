...
bool CPythonSystem::LoadConfig()
{
	...
	if (m_Config.bWindowed)
	{
		...
		// }
		if (m_Config.height >= screen_height) // Ajusta a janela do cliente.
		{
...
{
	FILE * File;
	File = fopen("conf/interface.cfg", "rb");
...
void CPythonSystem::SaveInterfaceStatus()
{
	...
	File = fopen("conf/interface.cfg", "wb");

	if (!File)
	{
		TraceError("Cannot open conf/interface.cfg");
		return;
	}
