...
void CPythonApplication::RenderGame()
{
	if (!PERF_CHECKER_RENDER_GAME)
	{
		...
		m_kChrMgr.Deform();
		m_kEftMgr.Update();	// Owsap render optimalization
...
bool CPythonApplication::Process()
{
...
#ifdef ENABLE_BONUS_SWITCHER
	static DWORD s_bsNextRefreshTime = ELTimer_GetMSec();
	if (ELTimer_GetMSec() > s_bsNextRefreshTime)
	{
		m_pyBonusSwitcher.Update();
		s_bsNextRefreshTime += m_pyBonusSwitcher.GetSwitchDelay();
	}
#endif
	...
	if (!s_bFrameSkip)
	{
		...
		if (!canRender)
		{
#ifdef ENABLE_BLACK_SCREEN_FIX
			RenderSpecial();
			SkipRenderBuffering(1500);
#else
			SkipRenderBuffering(3000);
#endif
		}
...
#ifdef ENABLE_SHINING_SYSTEM
	snprintf(szShiningTable, sizeof(szShiningTable), "locale/common/shining_table.txt", localePath);
#endif
...
#ifdef ENABLE_CUSTOM_SPECULAR_MAP
	CGrannyMaterial::CreateSphereMap(2, "d:/ymir work/special/spheremap_rainbow_armor.jpg");
	CGrannyMaterial::CreateSphereMap(3, "d:/ymir work/special/spheremap_rainbow_weapon.jpg");
#endif
	return true;
}
