...
PyObject * appSetDefaultFontName(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!PyTuple_GetBoolean(poArgs, 1, &bRefresh))
		bRefresh = false;

	DefaultFont_SetName(szFontName, bRefresh);
#else
...
#ifdef __CAPTCHA__
PyObject* appEncryptText(PyObject* poSelf, PyObject* poArgs)
{
	char* iString;
	if (!PyTuple_GetString(poArgs, 0, &iString))
		return Py_BuildException();

	std::string key("");

	char* iKey;
	if (PyTuple_GetString(poArgs, 1, &iKey))
		key = iKey;

	std::string stringText(iString);
	return Py_BuildValue("s", key.length() > 0 ? encrypt(stringText, key).c_str() : encrypt(stringText).c_str());
}

PyObject* appDecryptText(PyObject* poSelf, PyObject* poArgs)
{
	char* iString;
	if (!PyTuple_GetString(poArgs, 0, &iString))
		return Py_BuildException();
	...
	std::string stringText(iString);
	return Py_BuildValue("s", key.length() > 0 ? decrypt(stringText, key).c_str() : decrypt(stringText).c_str());
}
#endif
...
void initapp()
{
...
#ifdef __CAPTCHA__
		{ "EncryptText", 				appEncryptText,					METH_VARARGS },
		{ "DecryptText", 				appDecryptText,					METH_VARARGS },
#endif
		{ NULL, NULL },
	};
...
#ifdef ENABLE_ARMOR_EFFECT
	PyModule_AddIntConstant(poModule, "ENABLE_ARMOR_EFFECT",	1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_ARMOR_EFFECT",	0);
#endif
	...
	PyModule_AddIntConstant(poModule, "ENABLE_NEW_ATTRBONUS",	1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_NEW_ATTRBONUS",	0);
#endif

#ifdef ENABLE_EXTEND_SHOP_SYSTEM
	PyModule_AddIntConstant(poModule, "ENABLE_EXTEND_SHOP_SYSTEM",	1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_EXTEND_SHOP_SYSTEM",	0);
#endif
	...
	PyModule_AddIntConstant(poModule, "ENABLE_VIEW_STONE_IN_MAP",	1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_VIEW_STONE_IN_MAP",	0);
#endif
...
#ifdef ENABLE_NEW_LOGIN_INTERFACE
	PyModule_AddIntConstant(poModule, "ENABLE_NEW_LOGIN_INTERFACE", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_NEW_LOGIN_INTERFACE", 0);
#endif
	...
	PyModule_AddIntConstant(poModule, "ENABLE_ACHIEVEMENT_SYSTEM",	1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_ACHIEVEMENT_SYSTEM",	0);
#endif
...
#endif
#ifdef ENABLE_HIDE_COSTUME_SYSTEM_WEAPON
		PyModule_AddIntConstant(poModule, "ENABLE_HIDE_COSTUME_SYSTEM_WEAPON",	1);
#else
		PyModule_AddIntConstant(poModule, "ENABLE_HIDE_COSTUME_SYSTEM_WEAPON",	0);
#endif
#else
...
#ifdef ENABLE_SELECT_COLOR_TARGET
	PyModule_AddIntConstant(poModule, "ENABLE_SELECT_COLOR_TARGET",	1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_SELECT_COLOR_TARGET",	0);
#endif
	...
	PyModule_AddIntConstant(poModule, "ENABLE_CHEQUE_TO_GOLD",	1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_CHEQUE_TO_GOLD",	0);
#endif
...
#ifdef BODY_COSTUME_WITH_EFFECT_IMPERIAL
	PyModule_AddIntConstant(poModule, "BODY_COSTUME_WITH_EFFECT_IMPERIAL",	1);
#else
	PyModule_AddIntConstant(poModule, "BODY_COSTUME_WITH_EFFECT_IMPERIAL",	0);
#endif
...
#ifdef ENABLE_MAX_FPS
	PyModule_AddIntConstant(poModule, "ENABLE_MAX_FPS",	1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_MAX_FPS",	0);
#endif

#ifdef ENABLE_SYSTEM_MT2
	PyModule_AddIntConstant(poModule, "ENABLE_SYSTEM_MT2",	1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_SYSTEM_MT2",	0);
#endif

#ifdef ENABLE_SVSIDE
	PyModule_AddIntConstant(poModule, "ENABLE_SVSIDE",	1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_SVSIDE",	0);
#endif

#ifdef ENABLE_GUILD_STATISTICS
	PyModule_AddIntConstant(poModule, "ENABLE_GUILD_STATISTICS",	1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_GUILD_STATISTICS",	0);
#endif

#ifdef ENABLE_WAR_KILL_COUNTER
	PyModule_AddIntConstant(poModule, "ENABLE_WAR_KILL_COUNTER",	1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_WAR_KILL_COUNTER",	0);
#endif

#ifdef ENABLE_WAR_INSIGNIA
	PyModule_AddIntConstant(poModule, "ENABLE_WAR_INSIGNIA",	1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_WAR_INSIGNIA",	0);
#endif
...
#ifdef WEAPON_COSTUME_CHRISTMAS_EFFECT
	PyModule_AddIntConstant(poModule, "WEAPON_COSTUME_CHRISTMAS_EFFECT",	1);
#else
	PyModule_AddIntConstant(poModule, "WEAPON_COSTUME_CHRISTMAS_EFFECT",	0);
#endif
	...
	PyModule_AddIntConstant(poModule, "ENABLE_EXTRA_SECURITY", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_EXTRA_SECURITY", 0);
#endif
	...
	PyModule_AddIntConstant(poModule, "ENABLE_BONUS_SWITCHER", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_BONUS_SWITCHER", 0);
#endif
...
#ifdef ENABLE_DYNAMIC_FONT
	PyModule_AddIntConstant(poModule, "ENABLE_DYNAMIC_FONT", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_DYNAMIC_FONT", 0);
#endif

#if defined(ENABLE_DAILY_GIFT_SYSTEM)
	PyModule_AddIntConstant(poModule, "ENABLE_DAILY_GIFT_SYSTEM", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_DAILY_GIFT_SYSTEM", 0);
#endif
	...
	PyModule_AddIntConstant(poModule, "BL_KILL_BAR", 1);
#else
	PyModule_AddIntConstant(poModule, "BL_KILL_BAR", 0);
#endif

#ifdef __IMPROVED_GUILD_WAR__
	PyModule_AddIntConstant(poModule, "__IMPROVED_GUILD_WAR__", 1);
#else
	PyModule_AddIntConstant(poModule, "__IMPROVED_GUILD_WAR__", 0);
#endif
...
#ifdef ENABLE_QUEST_WIDTH_EXPANSION
	...
	PyModule_AddIntConstant(poModule, "ENABLE_PVP_EFFECT_SYSTEM", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_PVP_EFFECT_SYSTEM", 0);
#endif
	...
	PyModule_AddIntConstant(poModule, "ENABLE_SHOP_RENEWAL", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_SHOP_RENEWAL", 0);
#endif
	...
	PyModule_AddIntConstant(poModule, "ENABLE_ASLAN_BUFF_NPC_SYSTEM", 1);
	#ifdef ASLAN_BUFF_NPC_ENABLE_EXPORT
		PyModule_AddIntConstant(poModule, "ASLAN_BUFF_NPC_ENABLE_EXPORT", 1);
	#else
		PyModule_AddIntConstant(poModule, "ASLAN_BUFF_NPC_ENABLE_EXPORT", 0);
	#endif
	#ifdef ASLAN_BUFF_NPC_ENABLE_EXPORT_COST
		PyModule_AddIntConstant(poModule, "ASLAN_BUFF_NPC_ENABLE_EXPORT_COST", 1);
	#else
		PyModule_AddIntConstant(poModule, "ASLAN_BUFF_NPC_ENABLE_EXPORT_COST", 0);
	#endif
	#ifdef ASLAN_BUFF_NPC_USE_SKILL_TECH_LEVEL
		PyModule_AddIntConstant(poModule, "ASLAN_BUFF_NPC_USE_SKILL_TECH_LEVEL", 1);
	#else
		PyModule_AddIntConstant(poModule, "ASLAN_BUFF_NPC_USE_SKILL_TECH_LEVEL", 0);
	#endif
#else
	PyModule_AddIntConstant(poModule, "ENABLE_ASLAN_BUFF_NPC_SYSTEM", 0);
#endif
...
#ifdef ENABLE_GM_LIST
	PyModule_AddIntConstant(poModule, "ENABLE_GM_LIST", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_GM_LIST", 0);
#endif

#ifdef PLAYERS_ONLINE_LIST_TO_GM
	PyModule_AddIntConstant(poModule, "PLAYERS_ONLINE_LIST_TO_GM", 1);
#else
	PyModule_AddIntConstant(poModule, "PLAYERS_ONLINE_LIST_TO_GM", 0);
#endif

#ifdef ENABLE_BARABARABARA_BEREBEREBERE
	PyModule_AddIntConstant(poModule, "ENABLE_BARABARABARA_BEREBEREBERE", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_BARABARABARA_BEREBEREBERE", 0);
#endif

#ifdef __CAPTCHA__
	PyModule_AddIntConstant(poModule, "__CAPTCHA__", 1);
#else
	PyModule_AddIntConstant(poModule, "__CAPTCHA__", 0);
#endif
...
#ifdef WORLD_BOSS_YUMA
	PyModule_AddIntConstant(poModule, "WORLD_BOSS_YUMA", 1);
#else
	PyModule_AddIntConstant(poModule, "WORLD_BOSS_YUMA", 0);
#endif
	...
	PyModule_AddIntConstant(poModule, "ENABLE_SET_CUSTOM_ATTRIBUTE_SYSTEM", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_SET_CUSTOM_ATTRIBUTE_SYSTEM", 0);
#endif
