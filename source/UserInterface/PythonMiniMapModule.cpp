...
PyObject* miniMapSetAutoHuntStatus(PyObject* poSelf, PyObject* poArgs)
{
	bool bStatus;
	if (!PyTuple_GetBoolean(poArgs, 0, &bStatus))
		return Py_BuildException();
	CPythonMiniMap::Instance().SetAutoHuntStatus(bStatus);
	return Py_BuildNone();
}
...
PyObject * miniMapGetAtlasXY(PyObject * poSelf, PyObject * poArgs)
{
	float fScrrenX;
	if (!PyTuple_GetFloat(poArgs, 0, &fScrrenX))
		return Py_BuildException();
	...
	float fPosX = 0.0f;
	float fPosY = 0.0f;
	bool bFind = CPythonMiniMap::Instance().GetAtlasXY(fScrrenX, fScrrenY, &fPosX, &fPosY);
	int iPosX, iPosY;
	PR_FLOAT_TO_INT(fPosX, iPosX);
	PR_FLOAT_TO_INT(fPosY, iPosY);
	iPosX /= 100;
	iPosY /= 100;
	return Py_BuildValue("iii", (int)bFind, iPosX, iPosY);
}
#endif
...
void initMiniMap()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "SetAutoHuntStatus",						miniMapSetAutoHuntStatus,								METH_VARARGS },
		{ "SetAutoHuntRange",						miniMapSetAutoHuntRange,								METH_VARARGS },
#endif

#ifdef ENABLE_BATTLE_ROYALE
		{ "GetAtlasXY",						miniMapGetAtlasXY,								METH_VARARGS },
#endif
