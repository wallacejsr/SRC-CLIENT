...
{
	if(!std::ifstream("conf/filter_history.cfg", std::ios::in).is_open())
		return;
	...
	try
	{
		cfg.readFile("conf/filter_history.cfg");
	}
...
{
	if(!std::ifstream("conf/filter_patterns.cfg", std::ios::in).is_open())
		return;
	...
	try
	{
		cfg.readFile("conf/filter_patterns.cfg");
	}
	catch(const FileIOException &fioex)
	{
		UNUSED_VAR(fioex);
...
void CPythonOfflineshop::SaveFilterHistory()
{
	...
	try
	{
		cfg.writeFile("conf/filter_history.cfg");
	}

	catch(const FileIOException &fioex)
	{
		UNUSED_VAR(fioex);
		PutsError( "I/O error while writing file: conf/filter_history.cfg");
	}
}
...
void CPythonOfflineshop::SaveFilterPatterns()
{
	...
	try
	{
		cfg.writeFile("conf/filter_patterns.cfg");
	}

	catch(const FileIOException &fioex)
	{
		UNUSED_VAR(fioex);
		PutsError( "I/O error while writing file: conf/filter_patterns.cfg");
	}
}
...
{
	return m_bIsShowName && m_bIsShowShops;
}
...
bool CPythonOfflineshop::GetShowShopFlag()
{
	return m_bIsShowShops;
}


void CPythonOfflineshop::SetShowShopFlag(bool flag)
{
	m_bIsShowShops = flag;
}
...
PyObject* offlineshopHideShops(PyObject* poSelf, PyObject* poArgs)
{
	CPythonOfflineshop::instance().SetShowShopFlag(false);
	return Py_BuildNone();
}



PyObject* offlineshopShowShops(PyObject* poSelf, PyObject* poArgs)
{
	CPythonOfflineshop::instance().SetShowShopFlag(true);
	return Py_BuildNone();
}
#endif
...
void initofflineshop()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "HideShops",							offlineshopHideShops,					METH_VARARGS },
		{ "ShowShops",							offlineshopShowShops,					METH_VARARGS },
#endif
