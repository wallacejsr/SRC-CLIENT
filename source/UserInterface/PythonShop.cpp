...
#ifdef ENABLE_CHEQUE_SYSTEM
	SellingItem.cheque_price = dwChequePrice;
#endif
...
PyObject * shopGetItemCheque(PyObject * poSelf, PyObject * poArgs)
{
	...
	const TShopItemData * c_pItemData;
	if (CPythonShop::Instance().GetItemData(iIndex, &c_pItemData))
		return Py_BuildValue("b", c_pItemData->price.dwChequePrice);
...
PyObject * shopGetItemAchievement(PyObject * poSelf, PyObject * poArgs)
{
	int iIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iIndex))
		return Py_BuildException();
	...
	const TShopItemData * c_pItemData;
	if (CPythonShop::Instance().GetItemData(iIndex, &c_pItemData))
		return Py_BuildValue("i", c_pItemData->achievement_price);
...
PyObject * shopGetBuyWithItem(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!PyTuple_GetInteger(poArgs, 0, &iIndex))
		return Py_BuildException();
	int iBuyWithItemIndex;
	if (!PyTuple_GetInteger(poArgs, 1, &iBuyWithItemIndex))
		return Py_BuildException();

	const TShopItemData * c_pItemData;
	if (CPythonShop::Instance().GetItemData(iIndex, &c_pItemData))
		return Py_BuildValue("i", c_pItemData->dwBuyWithItem[iBuyWithItemIndex]);
...
PyObject * shopGetBuyWithItemCount(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!PyTuple_GetInteger(poArgs, 0, &iIndex))
		return Py_BuildException();
	int iBuyWithItemCountIndex;
	if (!PyTuple_GetInteger(poArgs, 1, &iBuyWithItemCountIndex))
		return Py_BuildException();

	const TShopItemData * c_pItemData;
	if (CPythonShop::Instance().GetItemData(iIndex, &c_pItemData))
		return Py_BuildValue("i", c_pItemData->dwBuyWithItemCount[iBuyWithItemCountIndex]);
...
#endif
#ifdef ENABLE_ACHIEVEMENT_SYSTEM
		{ "GetItemAchievement",			shopGetItemAchievement,			METH_VARARGS },
#endif
...
#ifdef ENABLE_SHOP_RENEWAL
	PyModule_AddIntConstant(poModule, "SHOP_BUY_ITEM_COUNT", BUY_WITH_ITEM_MAX_NUM);
#endif
