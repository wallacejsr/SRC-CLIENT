...
PyObject* netSendRankingListPacket(PyObject* poSelf, PyObject* poArgs)
{
	BYTE byValue = 0;
	if (!PyTuple_GetInteger(poArgs, 0, &byValue))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendRankingListPacket(byValue);
	return Py_BuildNone();
}
#endif
...
PyObject* netSendDailyGift(PyObject* poSelf, PyObject* poArgs)
{
	BYTE bAction = DAILY_GIFT_CLOSE;
	BYTE bSlotIndex = 0;
	...
	case 2:
		if (!PyTuple_GetByte(poArgs, 0, &bAction))
			return Py_BuildException();
		if (!PyTuple_GetByte(poArgs, 1, &bSlotIndex))
			return Py_BuildException();
		break;
	default:
		return Py_BuildException();
	...
	net.SendDailyGift(bAction, bSlotIndex);
...
PyObject* netSendBuffNPCAction(PyObject* poSelf, PyObject* poArgs)
{
	...
	int dValue2;
	if (!PyTuple_GetInteger(poArgs, 3, &dValue2))
		return Py_BuildException();

	int dValue3;
	if (!PyTuple_GetInteger(poArgs, 4, &dValue3))
		return Py_BuildException();

	CPythonNetworkStream::Instance().SendBuffNPCAction(bAction, dValue0, dValue1, dValue2, dValue3);
	return Py_BuildNone();
}
#endif

#if defined(ENABLE_SET_CUSTOM_ATTRIBUTE_SYSTEM)
PyObject* netSendSetCustomAttribute(PyObject* poSelf, PyObject* poArgs)
{
	TItemPos TargetPos;
	if (!PyTuple_GetInteger(poArgs, 0, &TargetPos.window_type))
		return Py_BuildException();

	if (!PyTuple_GetInteger(poArgs, 1, &TargetPos.cell))
		return Py_BuildException();

	PyObject* pObject;
	if (!PyTuple_GetObject(poArgs, 2, &pObject))
		return Py_BuildException();

	DWORD dwApplyNumList[ITEM_ATTRIBUTE_SLOT_NORM_NUM] = {};
	if (PyList_Check(pObject))
	{
		int iListSize = PyList_Size(pObject);
		for (int iAttributeIndex = 0; iAttributeIndex < iListSize; iAttributeIndex++)
		{
			if (iAttributeIndex > ITEM_ATTRIBUTE_SLOT_NORM_NUM)
				return Py_BuildException();

			PyObject* pObjectItem = PyList_GetItem(pObject, iAttributeIndex);
			dwApplyNumList[iAttributeIndex] = _PyInt_AsInt(pObjectItem);
		}
	}

	CPythonNetworkStream::Instance().SendSetCustomAttribute(TargetPos, dwApplyNumList);
	return Py_BuildNone();
}
#endif
...
void initnet()
{
	static PyMethodDef s_methods[] =
	{
		...
		#endif
		#ifdef ENABLE_RANKING_SYSTEM
		{ "SendRankingListPacket",						netSendRankingListPacket,				METH_VARARGS },
		#endif
		#ifdef ENABLE_HUNTING_SYSTEM
		{ "SendHuntingAction", netSendHuntingAction, METH_VARARGS },
		#endif
		#if defined(ENABLE_DAILY_GIFT_SYSTEM)
		{ "SendDailyGift", netSendDailyGift, METH_VARARGS },
		#endif
		#ifdef ENABLE_ASLAN_BUFF_NPC_SYSTEM
		...
		#endif
		#if defined(ENABLE_SET_CUSTOM_ATTRIBUTE_SYSTEM)
		{ "SendSetCustomAttribute", netSendSetCustomAttribute, METH_VARARGS },
		#endif
		{ NULL,										NULL,										NULL },
	};
	...
	PyModule_AddIntConstant(poModule, "CHARACTER_SLOT_COUNT_MAX", 5);
#else
	PyModule_AddIntConstant(poModule, "CHARACTER_SLOT_COUNT_MAX", 4);
#endif
	PyModule_AddIntConstant(poModule, "ACCOUNT_CHARACTER_SLOT_LAST_PLAYTIME", 0);
#endif
