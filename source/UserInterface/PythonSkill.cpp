...
bool CPythonSkill::SSkillData::GetStateBuffNPC(const char* c_szStateName, int* piState, int iMinMaxType, int intPoint)
{
	std::map<std::string, DWORD>* pStatusNameMap = NULL;
	...
	if (it != pStatusNameMap->end()) {
		if (intPoint != 0 && it->second == POINT_IQ)
			*piState = intPoint;
		else
			*piState = CPythonPlayer::Instance().GetStatus(it->second);
	}
...
float CPythonSkill::SSkillData::ProcessFormulaBuffNPC(CPoly* pPoly, float fSkillLevel, int iMinMaxType, int intPoint)
{
	if (pPoly->Analyze())
	{
		for (DWORD i = 0; i < pPoly->GetVarCount(); ++i)
		{
			...
			if (!strcmp("SkillPoint", c_szVarName) || !strcmp("k", c_szVarName)) {
				...
				int iState;
				if (!GetStateBuffNPC(c_szVarName, &iState, iMinMaxType, intPoint))
					return 0.0f;
...
const char* CPythonSkill::SSkillData::GetAffectBuffNPCDescription(DWORD dwIndex, float fSkillLevel, int intPoint)
{
	if (dwIndex >= AffectDataVector.size())
		return NULL;
	...
	float fMinValue = ProcessFormulaBuffNPC(&minPoly, fSkillLevel, 0, intPoint);
	float fMaxValue = ProcessFormulaBuffNPC(&maxPoly, fSkillLevel, 0, intPoint);
...
PyObject* skillGetBuffNPCSkillAffectDescription(PyObject* poSelf, PyObject* poArgs)
{
	int iSkillIndex;
	...
	int iIntPoint;
	if (!PyTuple_GetInteger(poArgs, 3, &iIntPoint))
		return Py_BadArgument();
	...
	return Py_BuildValue("s", c_pSkillData->GetAffectBuffNPCDescription(iAffectIndex, fSkillPoint, iIntPoint));
}
#endif
...
#ifdef ENABLE_ASLAN_BUFF_NPC_SYSTEM
		{ "GetBuffNPCSkillAffectDescription",		skillGetBuffNPCSkillAffectDescription,		METH_VARARGS },
#endif
