...
#ifdef ENABLE_BONUS_SWITCHER
	BYTE bBsPos = CBonusSwitcher::Instance().GetPosByCell(packet_item_update.Cell.cell);
	if (bBsPos != c_ItemSlot_Count)
	{
		CBonusSwitcher::Instance().SetUpdatePos(bBsPos, true);
	}
#endif
...
bool CPythonNetworkStream::SendShopSellPacketNew(WORD wSlot, DWORD dwCount)
{
	if(!__CanActMainInstance())
		return true;
	...
	Tracef(" SendShopSellPacketNew(wSlot=%d, dwCount=%d)\n", wSlot, dwCount);
...
bool CPythonNetworkStream::RecvAttributesBySet()
{
	TPacketGCAttributesBySet kPacket;
	if (!Recv(sizeof(kPacket), &kPacket))
		return false;

	PyObject* pList = PyList_New(0);

	unsigned int iPacketSize = (kPacket.wSize - sizeof(TPacketGCAttributesBySet));
	for (; iPacketSize > 0; iPacketSize -= sizeof(CPythonItem::TMaxAttributesBySetTable))
	{
		CPythonItem::TMaxAttributesBySetTable MaxAttributesBySetTable;
		if (!Recv(sizeof(MaxAttributesBySetTable), &MaxAttributesBySetTable))
			return false;

		if (MaxAttributesBySetTable.dwApplyNum == 0)
			continue;

		PyList_Append(pList, Py_BuildValue("(ii)", MaxAttributesBySetTable.dwApplyNum, MaxAttributesBySetTable.lMaxValue));
	}

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "AttributesBySet", Py_BuildValue("iiO", kPacket.wItemPos, kPacket.bySuccessPct, pList));

	Py_DECREF(pList);
	return true;
}

bool CPythonNetworkStream::SendSetCustomAttribute(const TItemPos TargetPos, const DWORD ApplyNumList[])
{
	TPacketCGSetCustomAttribute Packet;
	Packet.byHeader = HEADER_CG_ATTRIBUTES_BY_SET;
	Packet.TargetPos = TargetPos;

	unsigned int uiApplyNumListSize = sizeof(ApplyNumList);
	if (uiApplyNumListSize > ITEM_ATTRIBUTE_SLOT_NORM_NUM)
		return false;

	for (int iAttributeIndex = 0; iAttributeIndex < ITEM_ATTRIBUTE_SLOT_NORM_NUM; ++iAttributeIndex)
		Packet.dwApplyNum[iAttributeIndex] = ApplyNumList[iAttributeIndex];
