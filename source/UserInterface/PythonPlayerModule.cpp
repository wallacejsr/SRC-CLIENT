...
PyObject * playerComboAttack(PyObject* poSelf, PyObject* poArgs)
{
	...
	int dummy;
	if (!PyTuple_GetInteger(poArgs, 0, &dummy))
	{
		PostQuitMessage(0);
		return Py_BuildNone();
	}
...
PyObject * playerGetAchievement(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("i", CPythonPlayer::Instance().GetStatus(POINT_ACHIEVEMENT));
}
#endif

#ifdef ENABLE_SYSTEM_RUNE
PyObject * playerGetPWar(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("b", CPythonPlayer::Instance().GetStatus(POINT_PWAR));
}

PyObject * playerGetPSura(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("b", CPythonPlayer::Instance().GetStatus(POINT_PSURA));
}

PyObject * playerGetPMob(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("b", CPythonPlayer::Instance().GetStatus(POINT_PMOB));
}

PyObject * playerGetPNinja(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("b", CPythonPlayer::Instance().GetStatus(POINT_PNINJA));
}

PyObject * playerGetPSaman(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("b", CPythonPlayer::Instance().GetStatus(POINT_PSAMAN));
}

PyObject * playerGetPpoint(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("b", CPythonPlayer::Instance().GetStatus(POINT_PPOINT));
}
#endif
...
PyObject * playerGetItemUnSealLeftTime(PyObject * poSelf, PyObject * poArgs)
{
	...
	const time_t time = CPythonPlayer::Instance().GetItemBind(Cell) - CPythonApplication::Instance().GetServerTimeStamp();
	int hours, minutes;
	minutes = (time / 60) % 60;
	hours = time / 60 / 60;
...
PyObject * playerCanSealItem(PyObject * poSelf, PyObject * poArgs)
{
	...
	case CItemData::ITEM_TYPE_COSTUME:
		if (CPythonPlayer::Instance().GetItemBind(TargetSlotIndex) == 0)
			return Py_BuildValue("i", TRUE);
		break;
	}
	return Py_BuildValue("i", FALSE);
...
PyObject * playerCanUnSealItem(PyObject * poSelf, PyObject * poArgs)
{
	...
	case CItemData::ITEM_TYPE_WEAPON:
	case CItemData::ITEM_TYPE_COSTUME:
		if (CPythonPlayer::Instance().GetItemBind(TargetSlotIndex) >= 1)
			return Py_BuildValue("i", TRUE);
		break;
	}
	return Py_BuildValue("i", FALSE);
...
PyObject * playerGetItemSealDate(PyObject* poSelf, PyObject* poArgs)
{
	...
	case 1:
	{
		...
		long time = CPythonPlayer::Instance().GetItemBind(TItemPos(INVENTORY, iSlotIndex));
	...
	case 2:
	{
		...
		long time = CPythonPlayer::Instance().GetItemBind(Cell);
		return Py_BuildValue("i", time);
	}
	default:
		return Py_BuildException();
...
PyObject* playerBloktanCikart(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (!xyBuldu)
	{
		CPythonChat::Instance().AppendChat(CHAT_TYPE_INFO, "Seu personagem nao pode ser salvo pelo sistema.");
		return Py_BuildValue("i", 0);
	}
	...
	CPythonChat::Instance().AppendChat(CHAT_TYPE_INFO, "Seu personagem foi resgatado com sucesso.");
	return Py_BuildValue("i", 1);
}
#endif
...
void initPlayer()
{
...
#endif
#ifdef ENABLE_ACHIEVEMENT_SYSTEM
		{ "GetAchievement",				playerGetAchievement,				METH_VARARGS },
#endif
#ifdef ENABLE_SYSTEM_RUNE
		{ "GetPWar",					playerGetPWar,						METH_VARARGS },
		{ "GetPSura",					playerGetPSura,						METH_VARARGS },
		{ "GetPMob",					playerGetPMob,						METH_VARARGS },
		{ "GetPNinja",					playerGetPNinja,					METH_VARARGS },
		{ "GetPSaman",					playerGetPSaman,					METH_VARARGS },
		{ "GetPpoint",					playerGetPpoint,					METH_VARARGS },
#endif
...
#endif
#ifdef ENABLE_ACHIEVEMENT_SYSTEM
	PyModule_AddIntConstant(poModule, "ACHIEVEMENT",			POINT_ACHIEVEMENT);
#endif
	...
	PyModule_AddIntConstant(poModule, "BUFF_EQUIPMENT", BUFF_EQUIPMENT);
	PyModule_AddIntConstant(poModule, "SLOT_TYPE_BUFF_EQUIPMENT", SLOT_TYPE_BUFF_EQUIPMENT);
	PyModule_AddIntConstant(poModule, "BUFF_EQUIPMENT_SLOT_START", c_Buff_Equipment_Slot_Start);
	PyModule_AddIntConstant(poModule, "BUFF_EQUIPMENT_SLOT_COUNT", CItemData::BUFF_WINDOW_SLOT_MAX_NUM);
#endif
	...
	PyModule_AddIntConstant(poModule, "ITEM_STONES_MAX_NUM", ITEM_STONES_MAX_NUM);
#endif
	...
	PyModule_AddIntConstant(poModule, "PK_MODE_BATALHA",			PK_MODE_BATALHA);
#endif
...
#ifdef ENABLE_CHEQUE_SYSTEM
	PyModule_AddIntConstant(poModule, "GOLD_MAX", 2000000000);
	PyModule_AddIntConstant(poModule, "CHEQUE_MAX", 10000);
#endif

#ifdef ENABLE_ACHIEVEMENT_SYSTEM
	PyModule_AddIntConstant(poModule, "ACHIEVEMENT_MAX", 100000);
#endif

// #if defined(ENABLE_SET_CUSTOM_ATTRIBUTE_SYSTEM)
	// PyModule_AddIntConstant(poModule, "ATTRIBUTE_NORM_NUM", ITEM_ATTRIBUTE_SLOT_NORM_NUM);
	// PyModule_AddIntConstant(poModule, "ATTRIBUTE_RARE_NUM", ITEM_ATTRIBUTE_SLOT_RARE_NUM);
// #endif
}