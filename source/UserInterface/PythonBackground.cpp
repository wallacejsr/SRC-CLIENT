...
#ifdef ENABLE_BATTLE_ROYALE
	if (m_bVisibleSafeArea)
		rkMap.RenderSafeArea();
#endif
}
...
#ifdef ENABLE_BATTLE_ROYALE
void CPythonBackground::ClearSafeArea()
{
	if (!IsMapReady())
		return;
...
void CPythonBackground::RegisterSafeArea(int isx, int isy, int iex, int iey)
{
	if (!IsMapReady())
		return;

	CMapOutdoor& rkMap = GetMapOutdoorRef();
	rkMap.RegisterSafeArea(isx, isy, iex, iey);
}

void CPythonBackground::VisibleSafeArea()
{
	if (!IsMapReady())
		return;

	CMapOutdoor& rkMap = GetMapOutdoorRef();
	rkMap.VisibleSafetyArea();

	m_bVisibleSafeArea = TRUE;
}

void CPythonBackground::DisableSafeArea()
{
	if (!IsMapReady())
		return;

	CMapOutdoor& rkMap = GetMapOutdoorRef();
	rkMap.DisableSafetyArea();
