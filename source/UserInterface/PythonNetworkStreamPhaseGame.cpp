...
#include "LocaleStringManager.h" //_novo_locale_string
...
void CPythonNetworkStream::GamePhase()
{
	...
	const DWORD MAX_RECV_COUNT = 32; // Alterado de 4 para 32
    ...
    while (ret)
	{
		...
		switch (header)
		{
			...
			case HEADER_GC_LOCALE_CHAT: //_novo_locale_string
			...
			case HEADER_GC_RANKING_LIST:
				ret = RecvRankingListPacket();
				break;
			#endif
			...
			case HEADER_GC_BATTLE_ROYALE:
				ret = RecvBattleRoyalePacket();
				break;
#endif

#if defined(ENABLE_SET_CUSTOM_ATTRIBUTE_SYSTEM)
			case HEADER_GC_ATTRIBUTES_BY_SET:
				ret = RecvAttributesBySet();
				break;
#endif
...
#if defined(ENABLE_SVSIDE) && defined(ENABLE_SVSIDE_PLUS)
	{ //SVSIDE_PLUS
...
bool CPythonNetworkStream::RecvLocaleChatPacket() //_novo_locale_string
{
...
bool CPythonNetworkStream::RecvPointChange()
{
	...
	if (pInstance && PointChange.dwVID == pInstance->GetVirtualID())
	{
		...
		if (POINT_ACHIEVEMENT == PointChange.Type)
		{
			if (PointChange.amount > 0)
			{
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnPickAchievement", Py_BuildValue("(i)", PointChange.amount));
			}
		}
#endif
// rune system
		if (POINT_PPOINT == PointChange.Type)
		{
			if (PointChange.amount > 0)
			{
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnPickPPoint", Py_BuildValue("(b)", PointChange.amount));
			}
		}
		if (POINT_PWAR == PointChange.Type)
		{
			if (PointChange.amount > 0)
			{
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnPickPWar", Py_BuildValue("(b)", PointChange.amount));
			}
		}
		if (POINT_PSURA == PointChange.Type)
		{
			if (PointChange.amount > 0)
			{
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnPickPSura", Py_BuildValue("(b)", PointChange.amount));
			}
		}
		if (POINT_PMOB == PointChange.Type)
		{
			if (PointChange.amount > 0)
			{
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnPickPMob", Py_BuildValue("(b)", PointChange.amount));
			}
		}
		if (POINT_PNINJA == PointChange.Type)
		{
			if (PointChange.amount > 0)
			{
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnPickPNinja", Py_BuildValue("(b)", PointChange.amount));
			}
		}
		if (POINT_PSAMAN == PointChange.Type)
		{
			if (PointChange.amount > 0)
			{
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnPickPSaman", Py_BuildValue("(b)", PointChange.amount));
			}
		}
// rune system
...
#ifdef ENABLE_CHEQUE_SYSTEM
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "ExchangeLogAppend", Py_BuildValue("(isississii)", logID, logData.owner, logData.ownerGold, logData.ownerIP, logData.target, logData.targetGold, logData.targetIP, logData.date, logData.ownerCheque, logData.targetCheque));
#else
...
bool CPythonNetworkStream::SendAttackPacket(UINT uMotAttack, DWORD fake1, DWORD fake2, DWORD dwVIDVictim, DWORD fake3)
{
	if (!__CanActMainInstance())
		return true;
...
//190112 Sanii: Updated BYTE byPos to WORD wPos
...
#ifdef ENABLE_RANKING_SYSTEM
bool CPythonNetworkStream::SendRankingListPacket(BYTE byValue)
{
	TPacketCGRankingListInfo kSendRankingListInfo;
	kSendRankingListInfo.byValue = byValue;

	if (!Send(sizeof(TPacketCGRankingListInfo), &kSendRankingListInfo))
		return false;
...
bool CPythonNetworkStream::RecvRankingListPacket()
{
	TPacketGCRankingListInfo p;
	if (!Recv(sizeof(TPacketGCRankingListInfo), &p))
	{
		Tracenf("RecvRankingSystem System Packet Error");
		return false;
	}

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_SetRankingList", Py_BuildValue("(iisiL)", p.byRankType, p.byLine, p.name, p.dwEmpire, p.iCount));
...
#if defined(ENABLE_DAILY_GIFT_SYSTEM)
bool CPythonNetworkStream::SendDailyGift(BYTE bAction, BYTE bSlotIndex)
{
	TPacketCGDailyGift packet;
	packet.bHeader = HEADER_CG_DAILY_GIFT;
	packet.bAction = bAction;
	packet.bSlotIndex = bSlotIndex;
	if (!Send(sizeof(packet), &packet))
		return false;
...
bool CPythonNetworkStream::RecvDailyGift()
{
	TPacketGCDailyGift packet;
	if (!Recv(sizeof(packet), &packet))
		return false;

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_ClearDailyGift", Py_BuildValue("()"));
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_DailyGiftCash", Py_BuildValue("(i)", packet.dwCash));
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_DailyGiftEndTime", Py_BuildValue("(i)", packet.dwEndTime));

	for (BYTE bDay = 0; bDay < DAILY_GIFT_WEEK_DAYS; ++bDay)
	{
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_RecieveDailyGift", Py_BuildValue("(iiiiiii)",
			(bDay + 1), packet.bWeek, packet.dwVnum[bDay], packet.dwCount[bDay], packet.dwCollectTime[bDay], packet.wCost[bDay], packet.bStatus[bDay]));
	}
...
#ifdef ENABLE_BATTLE_ROYALE
bool CPythonNetworkStream::RecvBattleRoyalePacket()
{
	TPacketGCBRSafeZone packet;
	if (!Recv(sizeof(packet), &packet))
		return false;

	CPythonMiniMap::Instance().RegisterBattleRoyaleZone(packet.startX, packet.startY, packet.endX, packet.endY, packet.bType);

	if (packet.bType == 1)
	{
		CPythonBackground & rkBG = CPythonBackground::Instance();
		rkBG.DisableSafeArea();
		rkBG.ClearSafeArea();
		rkBG.RegisterSafeArea(packet.startX,
			packet.startY,
			packet.endX,
			packet.endY);

		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BIN_BattleRoyaleRefreshTerrain", Py_BuildValue("()"));
	}
