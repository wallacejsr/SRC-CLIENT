...
const long c_lNormal_Waiting_Time = 0; /* @Leonardo - The oficial value is 10, but GF change it to 0 20190906 */
...
int CPythonEventManager::GetTotalLineCount(int iIndex)
{
	...
	if (!CEterPackManager::Instance().Get(File, pEventSet->szFileName, &pMap))
		return 0;

	int iTotalLine = 0;
	CMemoryTextFileLoader textFileLoader;
	textFileLoader.Bind(File.Size(), pMap);
	for (uint32_t i = 0; i < textFileLoader.GetLineCount(); ++i)
	{
		const std::string & c_rstrLineFile = textFileLoader.GetLineString(i);
		if (!c_rstrLineFile.empty() && c_rstrLineFile[0] != '[')
			iTotalLine++;
	}
