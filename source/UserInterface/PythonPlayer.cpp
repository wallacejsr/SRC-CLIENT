...
void CPythonPlayer::Update()
{
	...
	if (m_isDestPosition)
	{
		...
		if (pInstance)
		{
			...
			int distance = abs(int(pixelPosition.x) - m_ixDestPos) + abs(int(pixelPosition.y) - m_iyDestPos);
			if (distance < 10000)
			{
				m_isDestPosition = FALSE;
			}
			else if (CTimer::Instance().GetCurrentMillisecond() - m_iLastAlarmTime > 20000)
			{
				AlarmHaveToGo();
			}
		}
	...
	if (m_isConsumingStamina)
	{
		...
		m_fCurrentStamina -= (elapsedTime * m_fConsumeStaminaPerSec);
	...
	if (pInstance)
	{
		// pInstance de?i?keninin gecerli olup olmad©¥?©¥n©¥ kontrol ediyoruz.
		TPixelPosition PixelPosition;
		pInstance->NEW_GetPixelPosition(&PixelPosition);

		static IBackground& rkBG = IBackground::Instance(); // IBackground orne?i bir kez olu?turuluyor.
...
void CPythonPlayer::__Update_AutoAttack()
{
	...
	CInstanceBase* mainInstance = NEW_GetMainActorPtr();
	if (!mainInstance)
		return;
	...
	CInstanceBase* victimInstance = NEW_FindActorPtr(m_dwAutoAttackTargetVID);
	if (!victimInstance || victimInstance->IsDead() || (mainInstance->IsMountingHorse() && !mainInstance->CanAttackHorseLevel()) || !mainInstance->IsAttackableInstance(*victimInstance))
	{
		__ClearAutoAttackTargetActorID();
	}
	else if (mainInstance->IsSleep())
	{
...
const TItemData * CPythonPlayer::GetItemData(TItemPos Cell) const
{
	...
	case BUFF_EQUIPMENT:
		return &m_playerStatus.aBuffEquipmentItem[Cell.cell];
#endif
	default:
		return NULL;
	}
...
#ifdef ENABLE_ASLAN_BUFF_NPC_SYSTEM
	case BUFF_EQUIPMENT:
		m_playerStatus.aBuffEquipmentItem[Cell.cell] = c_rkItemInst;
		break;
#endif
	}
}
...
int CPythonPlayer::GetSpecialInventoryTypeByGlobalSlot(int iSlot)
{
	...
	if (iSlot >= 0 && iSlot < c_ItemSlot_Count)
	{
...
std::tuple<int, int> CPythonPlayer::GetSpecialInventoryRange(BYTE invenType)
{
	switch (invenType)
	{
	case INVENTORY_TYPE_INVENTORY:
		return std::tuple<int, int>(0, c_ItemSlot_Count);
	...
	if (pkInstMain)
	{
		...
		for (const auto& skillSlotNumber : m_vecAutoSkills)
		{
			...
			if (pSkillData)
			{
				...
				//if (!pkInstVictim || pkInstMain->IsWalking() || GetSkillCoolTime(skillSlotNumber) != 0)
