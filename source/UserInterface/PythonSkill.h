...
class CPythonSkill : public CSingleton<CPythonSkill>
{
	public:
		...
		typedef struct SSkillData
		{
			...
			bool GetStateBuffNPC(const char* c_szStateName, int* piState, int iMinMaxType = VALUE_TYPE_FREE, int intPoint = 0);
			float ProcessFormulaBuffNPC(CPoly* pPoly, float fSkillLevel = 0.0f, int iMinMaxType = VALUE_TYPE_FREE, int intPoint = 0);
			const char* GetAffectBuffNPCDescription(DWORD dwIndex, float fSkillLevel, int intPoint = 0);
#endif
