...
#ifdef ENABLE_BATTLE_ROYALE
void CMapOutdoor::ClearSafeArea()
{
	m_rkList_kSafeArea.clear();
}

void CMapOutdoor::RegisterSafeArea(int isx, int isy, int iex, int iey)
{
	RECT rect;
	rect.left = isx;
	rect.top = isy;
	rect.right = iex;
	rect.bottom = iey;
	m_rkList_kSafeArea.push_back(rect);
}

void CMapOutdoor::VisibleSafetyArea()
{
	std::map<int, BYTE*> kMap_pbyMarkBuf;
	std::set<int> kSet_iProcessedMapIndex;

	std::list<RECT>::iterator itorRect = m_rkList_kSafeArea.begin();
	for (; itorRect != m_rkList_kSafeArea.end(); ++itorRect)
	{
		const RECT & rkRect = *itorRect;
		...
		for (int ixCell = ix1Cell; ixCell <= ix2Cell; ++ixCell)
			for (int iyCell = iy1Cell; iyCell <= iy2Cell; ++iyCell)
			{
				...
				//FIX FCKING DUPLICATION BUG
				WORD kkX, kkY = 0;
				pTerrain->GetCoordinate(&kkX, &kkY);

				if (ixTerrain != kkX && iyTerrain != kkY)
					continue;
				//FIX FCKING DUPLICATION BUG
	...
	for (; itorTerrain != kMap_pbyMarkBuf.end(); ++itorTerrain)
	{
		...
		pTerrain->AllocateSafeSplats(pbyBuf);
	}

	stl_wipe_second(kMap_pbyMarkBuf);
}

void CMapOutdoor::DisableSafetyArea()
{
	for (int i = 0; i < AROUND_AREA_NUM; ++i)
	{
		if (!m_pTerrain[i])
		...
		m_pTerrain[i]->DeallocateSafeSplats();
	}
}
#endif
