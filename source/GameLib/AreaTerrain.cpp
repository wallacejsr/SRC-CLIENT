...
CTerrain::CTerrain()
{
	...
	memset(&m_lpSafetyTexture, 0, sizeof(m_lpSafetyTexture));
#endif
	Initialize();
}
...
#ifdef ENABLE_BATTLE_ROYALE
void CTerrain::AllocateSafeSplats(BYTE * pbyAlphaMap)
{
	TTerainSplat & rAttrSplat = m_SafeSplatPatch.Splats[0];
	HRESULT hr;

	if (m_lpSafetyTexture)
	{
		ULONG ulRef;
		do
		{
			ulRef = m_lpSafetyTexture->Release();
		} while (ulRef > 0);
	}

	do
	{
		hr = ms_lpd3dDevice->CreateTexture(ATTRMAP_XSIZE, ATTRMAP_YSIZE, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &m_lpSafetyTexture);
	} while (FAILED(hr));

	D3DLOCKED_RECT d3dlr;
	do
	{
		hr = m_lpSafetyTexture->LockRect(0, &d3dlr, 0, 0);
	} while (FAILED(hr));
	...
	do
	{
		hr = m_lpSafetyTexture->UnlockRect(0);
	} while (FAILED(hr));

	rAttrSplat.pd3dTexture = m_lpSafetyTexture;
	m_bSafety = true;
}

void CTerrain::DeallocateSafeSplats()
{
	TTerainSplat & rSplat = m_SafeSplatPatch.Splats[0];
	if (m_lpSafetyTexture)
	{
		ULONG ulRef;
		do
	...
	rSplat.pd3dTexture = NULL;
	m_lpSafetyTexture = NULL;
	m_bSafety = FALSE;

	memset(&m_SafeSplatPatch, 0, sizeof(m_SafeSplatPatch));
}
#endif
