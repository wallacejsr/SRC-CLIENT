...
bool CActorInstance::InterceptMotion(EMotionPushType iMotionType, WORD wMotion, float fBlendTime, UINT uSkill, float fSpeedRatio)
{
	...
	{ //SVSIDE_PLUS
		if (IsPoly()){
			fSpeedRatio = GetMotionDuration(dwMotionKey) / 1.1f * fSpeedRatio;
		}
	} //#SVSIDE_PLUS
#endif
...
#if defined(ENABLE_SVSIDE) && defined(ENABLE_SVSIDE_PLUS)
	//SVSIDE_PLUS
	DWORD dwRealMotionKey = 0;
	{
		if (IsPoly()){
			dwRealMotionKey = __SetMotion(kSetMotData, kSetMotData.dwMotKey);
		}
		else{
			dwRealMotionKey = __SetMotion(kSetMotData);
		}
	}
	//#SVSIDE_PLUS
#else
