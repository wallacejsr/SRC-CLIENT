...
std::list<CGraphicTextInstance*> CGraphicTextInstance::l_graphicTextInstance;

void CGraphicTextInstance::AddTextInstance(CGraphicTextInstance* t)
{
	l_graphicTextInstance.push_back(t);
}

void CGraphicTextInstance::RemoveTextInstance(const CGraphicTextInstance* t)
{
	const auto it = std::find(l_graphicTextInstance.begin(), l_graphicTextInstance.end(), t);

	if (it != l_graphicTextInstance.end())
		l_graphicTextInstance.erase(it);
}

void CGraphicTextInstance::ReloadTextInstance(CGraphicText* pkDefaultFont)
{
	for (const auto& it : l_graphicTextInstance)
		if (!it->IsDisableDynamicFont())
			it->SetTextPointer(pkDefaultFont);
}
#endif
...
{
#ifdef ENABLE_DYNAMIC_FONT
	const auto pkInst = ms_kPool.Alloc();
	AddTextInstance(pkInst);
	return pkInst;
#else
